...
import org.jcategory.strategy.StrategyInvocationHandler;
...
public class Category implements Serializable {
	 ...
	 */
	public <T> List<T> getLocalProperty(Key key) {
	...
	public boolean containsLocalProperty(Key key) {
		return !getLocalProperty(key).isEmpty();
	}
	...
	public <T> T getStrategy(Class<T> strategyInterface) {
		return getStrategy(key(strategyInterface), new Class[]{strategyInterface}, ChainOfResponsibility.DEFAULT_DELEGATION_EXCEPTION);
	}
	...
	public <T> T getStrategy(Class<T> strategyInterface, Class<? extends RuntimeException> exceptionClass) {
		return getStrategy(key(strategyInterface), new Class[]{strategyInterface}, exceptionClass);
	}
	...
	public <U> List<U> properties(Key key, Function<Category,List<Category>> linearizationFunction) {
		return newArrayList(new PropertyIterable(linearize(linearizationFunction), key));
	}
	...
	public <U> List<U> bottomUpProperties(Key key) {
		return newArrayList(new PropertyIterable(bottomUpCategories(), key));
	}
	...
	public <U> List<U> topDownProperties(Key key) {
		return newArrayList(new PropertyIterable(topDownCategories(), key));
	}
